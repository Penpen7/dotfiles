...
let
  ...
  tsParserDirs =
    pkgs.lib.pipe (vp.nvim-treesitter.withPlugins (p: builtins.attrValues p)).dependencies
      [
        (map toString)
  ...
  configDir = pkgs.runCommand "nvim-standalone-config" { } ''
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy.lua" {
        lazyNvim = vp.lazy-nvim;
      }
    } $out/nvim/lua/core/lazy.lua

    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/appearance.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/ui.lua" {
        fernVim = vp.vim-fern;
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/treesitter.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/lsp.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/edit.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/git.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/fzf.lua" {
    ...
    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/utils.lua" {
        vimStartuptime = vp.vim-startuptime;
      }
    } $out/nvim/lua/core/lazy/utils.lua

    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/ft/html.lua" {
        emmetVim = vp.emmet-vim;
      }
    } $out/nvim/lua/core/lazy/ft/html.lua

    install -m644 ${
      pkgs.replaceVars "${src}/lua/core/lazy/ft/markdown.lua" {
